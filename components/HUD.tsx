...
import { Trophy, Volume2, VolumeX, Zap, Users, ShieldAlert, Sparkles } from 'lucide-react';
...
export const HUD: React.FC<HUDProps> = ({
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-30 flex flex-col justify-between p-4 sm:p-6 select-none font-sans">
      {/* Top Bar: Session Timer, Stats, Controls */}
      <div className="flex items-start justify-between gap-4">
        {/* Left: Player Status & Score */}
        <div className="flex flex-col gap-2">
          {myPlayer && (
            <div className="pointer-events-auto flex items-center gap-3 rounded-2xl border border-slate-700/80 bg-slate-900/85 px-4 py-2.5 shadow-xl backdrop-blur-md">
              <div
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-3 text-sm font-bold text-white">
                  ...
                  <span className="text-amber-400">Kills: {myPlayer.kills}</span>
                </div>
              </div>

              {/* Active Power-up Badge */}
              {myPlayer.activePowerUp && (
                <div className="ml-2 flex items-center gap-1 rounded-lg bg-indigo-500/20 px-2 py-1 text-xs font-bold text-indigo-300 border border-indigo-500/40 animate-pulse">
          ...
          {/* Boost / Sprint Gauge */}
          {myPlayer && (
            <div className="pointer-events-auto flex w-48 flex-col gap-1 rounded-xl border border-slate-800 bg-slate-950/70 p-2 backdrop-blur-sm">
        ...
        <div className="pointer-events-auto flex flex-col items-center">
          <div
            className={`flex items-center gap-2.5 rounded-2xl border px-5 py-2 shadow-2xl backdrop-blur-xl transition-all duration-300 ${
              isUrgent
          ...
          <div className="mt-1.5 h-1 w-32 overflow-hidden rounded-full bg-slate-800">
            <div
        ...
        {/* Right: Sound Control & Kill Feed */}
        <div className="flex flex-col items-end gap-3">
          <div className="pointer-events-auto flex items-center gap-2">
            <button
              onClick={onToggleMute}
              className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-700/80 bg-slate-900/80 text-slate-300 shadow-lg backdrop-blur-md transition hover:border-slate-500 hover:text-white active:scale-95"
          ...
          <div className="flex flex-col gap-1.5">
            {gameState.killFeed.slice(-3).map((kill) => (
              <div
                key={kill.id}
                className="animate-in fade-in slide-in-from-right-3 flex items-center gap-2 rounded-lg border border-slate-800/80 bg-slate-950/80 px-3 py-1 text-xs backdrop-blur-sm"
              >
                {kill.killerName ? (
                  <>
                    <span
                  ...
                  <span className="text-slate-400">Self-crash:</span>
                )}
                <span
                  className="font-bold"
      ...
      {/* Bottom Bar: Emote Quick Selector, Leaderboard, Minimap */}
      <div className="flex items-end justify-between gap-4">
        ...
        <div className="hidden sm:flex items-center gap-2 rounded-xl bg-slate-950/60 px-3 py-1.5 text-[11px] font-medium text-slate-400 backdrop-blur-sm border border-slate-800/50">
